...
## Abstract
I will be exploring an IT support data set to determine if the age of an IT support interaction (i.e., the length of time it takes to resolve a ticket), the number of words exchanged between the analyst and the customer (i.e., the length of conversations measured in the number of words), the number of unique updates (i.e., the number of times the support ticket is updated) and the number of reassignments (i.e., the number of times an IT support ticket has been (re)assigned to the various technicians to resolve the support issue) contribute to the positive or negative sentiment associated with IT support interactions.

To perform this analysis, I am using a 2022 IT support tickets data set from a local liberal arts college in NYC containing IT support requests from faculty, staff and students. This dataset was sourced from my job (where I manage this group). As an IT support organization, the ability to detect positive and negative interactions between employees and users is crucial to the operations of the group; **identifying factors that contribute and increase positive sentiment are essential to customer satisfaction and employee retention efforts.**

A multiple regression analysis was performed against these specific variables because ticket age, number of updates, communication length and number of reassignments are controllable elements of a support interaction.  

**If the variables contribute to the sentiment score predication, it should be possible to dial up or dial down these elements to improve support interactions (i.e., increase their sentiment scores).**

## Abstract Summary

* As a manager of the IT support group for a liberal art college supporting faculty, staff and students...
...
* The data is an extract from an IT support tickets database containing 6390 support tickets from Jan 1 2022 to Nov 4 2022.

<br/>

* An IT support ticket is generated every time a user and technician/analyst interact to resolve a technical issue
...
* Age of ticket was calculated from the start and end dates for each ticket
...
* The analysis summarizes the total number of words, the total sentiment score, the total age, the number of updates and the number of times a ticket was reassigned
...
* achieves a certain level of negative, positive or neutral sentiment (the sentiment score)
...
1. Selected 4 predictor variables from the data set
2. Checked for linear relationships for each predictor variable against the response variable
3. Checked for collinearity among the predictor variables
4. Checked for variables with statistical significance with p < .05
...
## Collinearity check using Variable Inflation Factor (VIF)
...
```
## The profile and equation of sentiment for IT support tickets

The profile of a positive IT support tickets

* increasing number of words
* increasing number of update
* increasing number of reassignments
* increasing ticket
...
> y = 0.4468407 + 0.0066172 (total_words) +  0.0412394(update_count) +  0.2779674(reassignment_count) + 0.0041227(age_at_resolution_days)

## Intepretation & Conclusion

* With an **Adjusted R-squared: 0.712**, the multiple regression model is a *decent* predictor of sentiment.
